...
def compose_poster(cut_images: list[Path], lines: list[str],
                   positions: list[str | None] | None = None) -> str:
    """네컷 파일 + 대사 → 말풍선까지 구운 완성본 한 장. 돌려주는 값은 내려받을 URL이다.

    라우터(POST /api/storyboard/poster)가 쓰는 입구다. bubble()·compose() 는 PIL
    이미지를 다루는데 라우터가 들고 있는 건 파일 경로라, 그 사이를 여기서 잇는다.

    positions — 컷별 캐릭터 위치(director 의 shot.position). 있으면 말풍선을
    캐릭터 반대편에 둔다. 없으면 bubble_side 가 홀짝으로 떨어뜨린다.
    """
    import uuid
    if not cut_images:
    ...
    panels = []
    for i, path in enumerate(cut_images):
        im = Image.open(path)
        line = lines[i] if i < len(lines) else ""
        panels.append(bubble(im, line, bubble_side(pos[i] if i < len(pos) else None, i + 1)))
    out = compose(panels)
    name = f"{uuid.uuid4().hex}.png"
    out.save(settings.media_path / name, "PNG")
