...
3. **눈에 보이는 디테일을 세 가지 이상 담는다.** 외형이면 종류만 말하지 말고 색·몸집·
   얼굴 특징까지, 아웃핏이면 옷 종류·색·소품까지 적는다. 40자 안팎으로 쓴다.
4. 이미 채워진 칸과 모순되지 않아야 한다. 외형이 새인데 털 얘기를 하면 안 된다 —
   그 칸은 사장님이 정한 것이라 뒤엎지 않는다.

**참고한 게 있으면 무엇을 봤는지 알려준다.**

5. why — 왜 이 값을 떠올렸는지 사장님에게 한 문장으로 말한다. 존댓말, 40자 안팎.
   참고한 게 있으면 그걸 짚고, 없으면 그냥 어떤 느낌을 노렸는지 말한다.
6. basis — **why에서 근거로 든 것의 출처**다. 위 [가게]나 [캐릭터 시트]의 내용을
   조금이라도 참고했다면 그 줄을 **반드시** 인용한다. 최대 3개.
   - label: 그 줄의 이름 그대로 (업종 · 가게 소개 · 영업시간 · 주소 · 대표 상품 ·
     외형 · 아웃핏 · 설명 · 능력 · 나이 · 성별 · 이름)
   - quote: 그 줄에 **실제로 적혀 있는 말**에서 쓴 부분만 옮긴다. 요약하거나 바꿔
     쓰거나 없는 말을 지어내지 않는다.
   - **스스로 확인하라**: why에 가게나 시트 얘기(업종·상품·이미 적힌 칸)가 한 마디라도
     들어갔다면 basis가 비어 있으면 안 된다. 예를 들어 why에 "꽃집"이 나왔으면
     basis에 업종 줄을 넣는다.
   - 정말 아무것도 안 보고 떠올렸을 때만 빈 목록으로 둔다. 억지로 갖다 붙이지는
     않는다 — 근거가 없다고 제안을 취소하지도 않는다.

출력은 이 모양의 JSON만:
{"intent": "help", "proposal": "...",
 "basis": [{"label": "...", "quote": "..."}], "why": "..."}
"""
...
9. 아웃핏에 **앞치마·조끼·멜빵처럼 혼자 입을 수 없는 옷만** 적혔고 안에 입을 옷이
   없으면, 다음 칸으로 넘어가기 전에 안에 뭘 입을지 한 번 물어본다. **"그냥 털 위에
   걸친다"도 답이 된다**는 걸 같이 알려준다. 사장님이 정하지 않으면 그림 모델이 멋대로
   채워 넣기 때문이다 — 무엇을 입을지는 사장님이 정할 일이다.
10. **4문장을 넘기지 않는다.** 존댓말로, 동네 가게 사장님에게 말하듯 쉽게.
...
6. **어느 칸에서 뽑았는지 함께 낸다(basis).** label은 그 칸의 이름, quote는 그 칸에
   **실제로 적혀 있는 말**에서 쓴 부분이다. 요약하거나 지어내지 않는다. 최대 3개.
7. why — 그 말에서 이 키워드들이 어떻게 나왔는지 한 문장. 존댓말, 40자 안팎.

출력은 이 모양의 JSON만:
{"keywords": ["...", "..."], "basis": [{"label": "...", "quote": "..."}], "why": "..."}
"""

# 다 찬 시트에서 "외형 다시 하고 싶어" 같은 말을 받아내기 위한 것. 이게 없으면 사장님이
# 대화로는 아무것도 고칠 수 없고 시트에서 칸을 직접 눌러야만 했다.
_EDIT_SYSTEM = """\
사장님이 캐릭터 시트를 다 채운 뒤에 한 말이다. 이 말이 **어느 칸을 고치겠다는 뜻인지**
가려내 JSON으로 답한다.
...
규칙:
1. 특정 칸을 다시 정하고 싶다는 뜻이면 intent를 "edit"로, field에 그 칸의 키를 넣는다.
   ("외형 다시", "이름 바꾸고 싶어", "옷을 좀 고칠래", "성격이 마음에 안 들어")
...
def propose_field(char, field: str, text: str, store=None) -> dict:
    ...
    # 상세하게 받되 한 문단을 넘기면 제안이 아니라 소설이다. 승인 카드에도 안 들어간다.
