...
8. target — 사장님이 **지금 어느 칸 얘기를 하고 있는지** 그 키를 낸다.
   ...
   **부정·제외를 읽는다.** 한 문장에 칸 이름이 둘 나오고 한쪽이 "그거 말고"처럼
   제외되는 뜻이면, 제외된 쪽이 아니라 **남은 쪽**이 target 이다. 마지막에 나온
   이름을 고르는 게 아니라 뜻을 본다.
...
1. 특정 칸을 정하거나 다시 정하고 싶다는 뜻이면 intent를 "edit"로, field에 그 칸의
   ...
   **부정·제외를 읽는다.** 칸 이름이 둘 나오고 한쪽이 제외되는 뜻이면 제외된 쪽이
   아니라 남은 쪽을 낸다. 마지막에 나온 이름을 고르지 말고 뜻을 본다.
