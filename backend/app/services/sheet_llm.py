...
**참고한 게 있으면 무엇을 봤는지 알려준다.** 없으면 안 알려줘도 된다.

5. basis — 제안을 떠올릴 때 **실제로 참고한** 줄이 있으면 위 [가게]나 [캐릭터 시트]에서
   그대로 인용한다. 최대 3개. 참고한 게 없으면 **빈 목록으로 둔다** — 억지로 갖다 붙이지
   ...
   - quote: 그 줄에 **실제로 적혀 있는 말**에서 쓴 부분만 옮긴다. 요약하거나 바꿔
     쓰거나 없는 말을 지어내지 않는다.
6. why — 왜 이 값을 떠올렸는지 사장님에게 한 문장으로 말한다. 존댓말, 40자 안팎.
