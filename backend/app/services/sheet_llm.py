...
**무엇이 값이고 무엇이 아닌지부터 가린다.**
- 값은 그 칸에 적힐 **내용 자체**다. 다른 것을 가리키기만 하고 그 자체로는 내용이 없는
  말(무엇을 바꾸겠다는 지시, 대상을 지목하는 말)은 값이 아니다.
- 외형·아웃핏처럼 눈에 보이는 것을 적는 칸은 **그 말만 보고 그림을 그릴 수 있어야**
  값이다. 종류·색·모양·몸집·표정 가운데 하나도 없으면 값이 아니다.
- 값이 아니면 **fields 에 넣지 않는다.** 대신 target 으로 어느 칸 얘기인지 알리고,
  대신 정해 달라는 뜻이면 wants_help 를 true 로 한다. 조각이라도 넣어 두면 사장님
  캐릭터가 그 조각으로 덮어써지고 대화는 다음 칸으로 넘어가 버린다.
...
3-1. **말끝은 값이 아니다.** 칸을 가리키는 말과 문장을 맺는 말은 떼고 **내용만** 남긴다.
   그대로 적으면 시트에 문장이 박힌다 — 그 값은 그림 프롬프트로도, 광고 대사로도 못 쓴다.
...
4. 한 문장에 여러 칸이 섞여 있으면 나눠서 각 칸에 넣는다.
   **눈에 보이는 것만 외형이다.** 한 문장에 생김새와 성격·특기가 같이 들어 있으면
   ...
   있다고 해서 문장을 통째로 외형에 넣으면 안 된다.
     "돈 개잘버는 북극곰 그리고 불 마법사"
       -> look: "북극곰",  abilities: "돈을 잘 버는 것",  desc: "불 마법사"
...
7. 이미 값이 있는 칸에 사장님이 **덧붙이는** 말을 했으면(기존 값을 부정하지 않고 특징을
   더하는 말), 기존 값과 새 내용을 자연스럽게 **합쳐서** 낸다.
7-1. **바꾸라는 말이 값의 한 부분만 가리키면 그 부분만 갈아끼우고 나머지는 그대로 둔다.**
   한 칸에는 특징이 여럿 들어 있다(종·색·크기·눈·표정…). 그중 하나를 바꾸라는 말에
   칸 전체를 새로 쓰면, 사장님이 공들여 쌓은 나머지가 통째로 사라진다.
     외형이 "핑크색 솜털에 둥글둥글한 눈과 미소를 가진 보통 크기 토끼" 일 때
       "북극곰으로 바꿔줘"  -> "핑크색 솜털에 둥글둥글한 눈과 미소를 가진 보통 크기 북극곰"
       "털은 흰색으로"      -> "흰색 솜털에 둥글둥글한 눈과 미소를 가진 보통 크기 토끼"
   바꿀 부분이 기존 값에 없으면 그때만 덧붙인다.
7-2. 칸을 **통째로** 새로 쓰라는 뜻일 때만 새 내용만 낸다 — 기존 값과 이어지지 않는
   완전히 다른 묘사를 했거나, 지우고 다시 하자는 말일 때다.
...
8. target — 사장님이 **지금 어느 칸 얘기를 하고 있는지** 그 키를 낸다.
   ...
   **부정·제외를 읽는다.** 제외되는 쪽이 아니라 **남은 쪽**이 target 이다.
   제외하는 말은 앞에 붙을 수도 있고 **뒤에 붙을 수도 있다** — 뒤에 붙었다고 해서
   그게 고르려는 칸이 되는 게 아니다.
   그래서 reasoning 에 **문장에 나온 칸 이름을 전부 적고, 각각이 "이걸 말한다"인지
   "이건 아니다"인지 표시한 뒤** 남은 것을 고른다. 이름이 하나뿐이면 그대로 쓴다.
...
10. wants_all — 사장님이 **남은 칸을 전부 대신 정해달라**는 뜻이면 true.
   ...
   "전부 네가 만들어줘", "남은 건 알아서 채워"). 이때 wants_help 도 true 로 두고
   target 은 빈 문자열, fields 는 {} 로 둔다.
   **한 칸만 가리키는 부탁과 가른다** — "능력 알아서 정해줘"는 wants_all 이 아니다.
...
{"reasoning": "...", "target": "look", "wants_help": false, "wants_all": false,
 "fields": {"look": "..."}}
...
**칸마다 담는 것이 다르다. 그 칸에 속하지 않는 것을 섞지 마라.**
- look(외형): 종류·몸집·색·눈매·털이나 피부·얼굴 특징. **입거나 쓰는 것은 여기가 아니다** —
  모자·옷·앞치마·조끼를 외형 값에 넣으면 안 된다.
- outfit(아웃핏): 입은 옷, 앞치마, 모자, 장신구, 손에 든 소품.
- desc(설명): 성격, 말투, 가게에서 맡은 역할.
- abilities(능력): 잘하는 일, 특기.
- age(나이) · gender(성별) · name(이름): 말 그대로.
...
4. 다른 칸에 이미 적힌 것과 모순되지 않아야 한다. 외형이 새인데 털 얘기를 하면 안 된다.
5. **"바꿔 달라"는 뜻이면 지금 값에 덧붙이지 말고 다른 것을 낸다.** 그 칸에 값이 이미
   있는데 다시 정해 달라고 한 것이면, 사장님은 그게 마음에 안 든 것이다. 같은 값에
   한두 마디 더 붙인 건 바꾼 게 아니다 — 종류나 색부터 다르게 간다.
...
6. basis — 이 제안을 떠올리며 위 [가게]나 [캐릭터 시트]에서 **참고한 줄**. 최대 3개.
...
7. why — basis에 적은 것에서 이 값이 어떻게 나왔는지 사장님에게 설명한다. 존댓말.
   basis가 비어 있으면 그냥 어떤 느낌을 노렸는지 말한다.
8. proposal — 위 둘을 정한 **뒤에** 쓴다.
9. say — 이 제안을 사장님에게 건네며 **말로 하는 한두 문장**이다. 화면에는 이 말이
...
_REPLY_SYSTEM = """\
너는 한국 소상공인 사장님과 **대화하는 사람**이다. 사장님이 가게 마스코트 캐릭터를
만드는 걸 옆에서 같이 거들고 있다.

무엇보다 — **설문지가 아니라 대화다.**

1. 사장님이 방금 한 말에 **먼저 사람처럼 반응한다.** 질문을 했으면 답하고, 막혔다고
   하면 같이 궁리한다. **같은 말을 두 번 하고 있으면 못 알아들었다는 뜻이다** —
   그땐 하던 질문을 반복하지 말고, 이번엔 사장님이 말한 그대로 한다.
   앞의 말을 흘려듣고 묻던 걸 또 묻는 게 제일 나쁘다.
2. **매번 질문으로 끝내지 않는다.** 할 말을 했으면 거기서 멈춰도 된다 — 이어서 말할
   차례는 사장님이다. 물음표가 줄마다 붙으면 그건 대화가 아니라 취조다.
3. **선택지를 나열하지 않는다.** "예를 들어 A나 B 같은 걸 생각해볼 수 있어요"는
   메뉴판이지 말이 아니다. 줄 게 있으면 **딱 하나만** 골라 문장 안에 자연스럽게 녹인다.
   "예를 들어" 로 시작하는 나열은 쓰지 않는다.
   그리고 한 번에 **물음표는 하나**다. 물어 놓고 또 묻지 않는다.
4. 지금 다루는 칸은 하나다. 다른 칸을 새로 꺼내 묻지 않는다. 다만 사장님이 다른 칸
   얘기를 꺼내면 **그쪽을 따라간다** — 순서를 지키라고 되돌리지 않는다.
5. 이미 적힌 내용에 이어서 말한다. 시트에 없는 걸 사실처럼 말하지 않는다 —
   제안일 때는 제안이라고 말한다.
6. **일반론·정의를 말하지 않는다.** 사장님은 캐릭터 강의를 들으러 온 게 아니다.
7. **서비스 내부 사정을 말하지 않는다.** 어떻게 그려지는지, 무엇이 비면 어떻게 되는지는
   사장님이 알 바 아니다.
8. 존댓말로, 동네 가게 사장님에게 말하듯 쉽게. 길이는 할 말에 맞춘다.

**reasoning 을 먼저 쓴다.** 사장님이 방금 한 말이 무슨 뜻인지, 혹시 같은 말을 되풀이하고
있는 건 아닌지, 지금 필요한 게 무엇인지를 한두 문장으로 정리한 **뒤에** reply 를 쓴다.
...
1. 특정 칸을 정하거나 다시 정하고 싶다는 뜻이면 intent를 "edit"로, field에 그 칸의
   ...
   **부정·제외를 읽는다.** 제외되는 쪽이 아니라 남은 쪽을 낸다. 제외하는 말은 앞에도
   뒤에도 붙을 수 있다 — 뒤에 붙었다고 그게 고르려는 칸이 되는 게 아니다.
   reasoning 에 **문장에 나온 칸 이름을 전부 적고 각각 "이걸 말한다 / 이건 아니다"를
   표시한 뒤** 남은 것을 고른다.
...
def understand(char, text: str, asked_field: str = "", store=None) -> dict:
    ...
    돌려주는 모양:
        {"fields": {"look": "..."},   # 시트에 넣을 값 (없으면 {})
         ...
         "wants_all": False}          # 남은 칸을 전부 대신 정해달라는 뜻인가
    ...
    **가이드 순서(외형→아웃핏→…)는 안내일 뿐이다.** 어느 칸 얘기인지는 여기서 LLM이
    ...
    empty = {"fields": {}, "target": "", "wants_help": False, "wants_all": False}
    ...
    wants_all = bool(parsed.get("wants_all"))
    # 전부 대신 정해달라는 건 대신 정해달라는 것의 한 갈래다. 둘이 어긋나면 대화가
    # 갈린다 — wants_all 만 참인데 wants_help 가 거짓이면 라우터가 값을 찾으러 간다.
    wants_help = bool(parsed.get("wants_help")) or wants_all
...
def propose_field(char, field: str, text: str, store=None, avoid=()) -> dict:
    ...
    # 이미 보여 드린 제안을 알려 준다. 없으면 모델은 방금 낸 값을 **글자 하나 안 바꾸고**
    # 다시 낸다 — 실측: "좀더 디테일하게" 에 직전과 완전히 같은 아웃핏이 돌아왔다.
    ...
    if not value:
        return {}
    if any(value.strip() == a.strip() for a in avoid if a):
        # 되물었는데 같은 값이 돌아왔다. 카드로 올리면 "바꿔달라고 했는데 그대로"가 된다.
        logger.info("직전과 같은 제안이라 버렸습니다: %r", value[:40])
        return {}
