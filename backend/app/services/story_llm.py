...
6. 컷은 정확히 {n}개. **각 컷은 서로 다른 장면이다** — 같은 사실을 말만 바꿔 반복하지
   않는다. 1컷 대사를 사장님 문장 그대로 베끼지 않는다 — 그 순간을 장면으로 보여준다
   (예: "크루아상 15개 구웠어요" → 갓 구운 크루아상을 꺼내 보이는 장면). 이어지는 컷은
   그 상황의 디테일·반응·기대감으로 자연스럽게 이어지는 짧은 이야기 흐름을 만든다.
{meme_rule}
